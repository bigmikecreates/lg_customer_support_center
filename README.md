<div align='center'>

# Multi-Agent Customer Support System

![Env: Anaconda](https://img.shields.io/badge/Env-Anaconda-darkgreen)
![Agentic Ecosystem: Lang(Chain/Graph/Smith)](https://img.shields.io/badge/AI_Suite-Lang(Chain/Graph/Smith)-blue)

Demonstration of agentic capabilities **using the LangChain ecosystem** through the Customer Support industry.
</div>

<br>

## OVERVIEW

I will be exploring the below-listed frameworks from the LangChain ecosystem to design, orchestrate, and evaluate the agentic capabilities of this system:

- LangChain
- LangGraph
- LangSmith

The following document, taking inspiration from a human Customer Support Expert, is being used to define the persona for this multi-agent system (<a href='customer_rep_persona_store\customer_support_rep_persona.md'>link to persona definiton document</a>). This also means the multi-agent system’s performance will be benchmarked against that of a human Customer Support Representative.